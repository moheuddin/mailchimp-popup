<?php
  /*
  Plugin Name: MailChimp Popup with Exit Support
  Plugin URI: http://www.mailmunch.co
  Description: MailChimp Popup plugin allows you to quickly and easily add popup signup forms for your MailChimp lists. With exit-intent technology, you can convert exiting visitors.
  Version: 1.0.0
  Author: MailMunch
  ...
  */
  ...
  define( 'MAILCHIMP_POPUP_MAILMUNCH_SLUG', "mailchimp-popup-mailmunch");
  define( 'MAILCHIMP_POPUP_MAILMUNCH_VER', "1.0.0");
  define( 'MAILCHIMP_POPUP_MAILMUNCH_URL', "www.mailmunch.co");

  // Create unique WordPress instance ID
  if (get_option("mc_pop_mm_wordpress_instance_id") == "") {
    update_option("mc_pop_mm_wordpress_instance_id", uniqid());
  }

  // Adding Admin Menu
  add_action( 'admin_menu', 'mc_pop_mm_register_page' );

  function mc_pop_mm_register_page(){
    add_options_page('MailChimp', 'MailChimp', 'manage_options', MAILCHIMP_POPUP_MAILMUNCH_SLUG, 'mc_pop_mm_setup');
    $menu_page = add_menu_page( 'MailChimp Settings', 'MailChimp', 'manage_options', MAILCHIMP_POPUP_MAILMUNCH_SLUG, 'mc_pop_mm_setup', plugins_url( 'img/icon.png', __FILE__ ), 103.787 ); 
    // If successful, load admin assets only on that page.
    if ($menu_page) add_action('load-' . $menu_page, 'mc_pop_mm_load_plugin_assets');
  }

  function mc_pop_mm_plugin_settings_link($links) {
    $settings_link = '<a href="options-general.php?page='.MAILCHIMP_POPUP_MAILMUNCH_SLUG.'">Settings</a>';
    array_unshift($links, $settings_link);
    return $links;
  }

  $plugin = plugin_basename(__FILE__);
  add_filter('plugin_action_links_'.$plugin, 'mc_pop_mm_plugin_settings_link');

  function mc_pop_mm_load_plugin_assets() {
    add_action( 'admin_enqueue_scripts', 'mc_pop_mm_enqueue_admin_styles' );
    add_action( 'admin_enqueue_scripts', 'mc_pop_mm_enqueue_admin_scripts'  );
  }

  function mc_pop_mm_enqueue_admin_styles() {
    wp_enqueue_style(MAILCHIMP_POPUP_MAILMUNCH_SLUG . '-admin-styles', plugins_url( 'css/admin.css', __FILE__ ), array(), MAILCHIMP_POPUP_MAILMUNCH_VER );
  }

  function mc_pop_mm_enqueue_admin_scripts() {
    wp_enqueue_script(MAILCHIMP_POPUP_MAILMUNCH_SLUG . '-admin-script', plugins_url( 'js/admin.js', __FILE__ ), array( 'jquery' ), MAILCHIMP_POPUP_MAILMUNCH_VER );
  }

  // Adding MailMunch Asset Files (JS + CSS) 
  function mc_pop_mm_load_asset_code() {
    $mc_pop_mm_data = unserialize(get_option("mc_pop_mm_data"));
    if (!$mc_pop_mm_data["script_src"]) return;
    ...
    echo "</script>";
    echo('<script data-cfasync="false" src="//s3.amazonaws.com/mailmunch/static/site.js" id="mailmunch-script" data-mailmunch-site-id="'.$mc_pop_mm_data["site_id"].'" async></script>');
  }

  add_action('init', 'mc_pop_mm_assets');

  function mc_pop_mm_assets() {
    $mc_pop_mm_data = unserialize(get_option("mc_pop_mm_data"));
    if (count($mc_pop_mm_data) == 0) return;

    if (function_exists('wp_head')) {
      add_action( 'wp_head', 'mc_pop_mm_load_asset_code' ); 
    }
    elseif (function_exists('wp_footer')) {
      add_action( 'wp_footer', 'mc_pop_mm_load_asset_code' ); 
    }
  }

  function mc_pop_mm_add_post_containers($content) {
    if (is_single() || is_page()) {
      $content = mc_pop_mm_insert_form_after_paragraph("<div class='mailmunch-forms-in-post-middle' style='display: none !important;'></div>", "middle", $content);
  ...
  function mc_pop_mm_register_sidebar_widget() {
      register_widget( 'MC_POP_MM_Sidebar_Widget' );
  }
  add_action( 'widgets_init', 'mc_pop_mm_register_sidebar_widget' );

  function mc_pop_mm_insert_form_after_paragraph($insertion, $paragraph_id, $content) {
    $closing_p = '</p>';
  ...
  add_filter( 'the_content', 'mc_pop_mm_add_post_containers' );

  function mc_pop_mm_shortcode_form($atts) {
    return "<div class='mailmunch-forms-short-code mailmunch-forms-widget-".$atts['id']."' style='display: none !important;'></div>";
  }

  add_shortcode('mailmunch-form', 'mc_pop_mm_shortcode_form');

  function mc_pop_mm_setup() {
    $mm_helpers = new MailchimppopupMailmunchHelpers();
    $mc_pop_mm_data = unserialize(get_option("mc_pop_mm_data"));
    $mc_pop_mm_data["site_url"] = home_url();
    $mc_pop_mm_data["site_name"] = get_bloginfo();
    update_option("mc_pop_mm_data", serialize($mc_pop_mm_data));

    // This is a POST request. Let's save data first.
    if ($_POST) {
      $post_data = (isset($_POST["mc_pop_mm_data"]) ? $_POST["mc_pop_mm_data"] : array());
      $post_action = $_POST["action"];

      if ($post_action == "save_settings") { 

        $mc_pop_mm_data = array_merge(unserialize(get_option('mc_pop_mm_data')), $post_data);
        update_option("mc_pop_mm_data", serialize($mc_pop_mm_data));
      
      } else if ($post_action == "sign_in") {

        $mm = new MailchimppopupMailmunchApi($_POST["email"], $_POST["password"], "http://".MAILCHIMP_POPUP_MAILMUNCH_URL);
        if ($mm->validPassword()) {
          update_option("mc_pop_mm_user_email", $_POST["email"]);
          update_option("mc_pop_mm_user_password", base64_encode($_POST["password"]));
        ...
        if (empty($_POST["email"]) || empty($_POST["password"])) {
          ...
          $mc_pop_mm_email = $account_info['email'];
          $mc_pop_mm_password = $account_info['password'];

          $mm = new MailchimppopupMailmunchApi($mc_pop_mm_email, $mc_pop_mm_password, "http://".MAILCHIMP_POPUP_MAILMUNCH_URL);
          if ($mm->isNewUser($_POST['email'])) {
            $update_result = $mm->updateGuest($_POST['email'], $_POST['password']);
            $result = json_decode($update_result['body']);
            update_option("mc_pop_mm_user_email", $result->email);
            update_option("mc_pop_mm_user_password", base64_encode($_POST['password']));
            if (!$result->guest_user) { delete_option("mc_pop_mm_guest_user"); }
            $mc_pop_mm_email = $result->email;
            $mc_pop_mm_password = $_POST['password'];

            // We have update the guest with real email address, let's create a site now
            $mm = new MailchimppopupMailmunchApi($mc_pop_mm_email, $mc_pop_mm_password, "http://".MAILCHIMP_POPUP_MAILMUNCH_URL);

            $update_result = $mm->updateSite($mc_pop_mm_data["site_name"], $mc_pop_mm_data["site_url"]);
            $result = json_decode($update_result['body']);
            $mc_pop_mm_data = unserialize(get_option("mc_pop_mm_data"));
            $mc_pop_mm_data["site_url"] = $result->domain;
            $mc_pop_mm_data["site_name"] = $result->name;
            update_option("mc_pop_mm_data", serialize($mc_pop_mm_data));
          } else {
            $user_exists = true;
          }
        ...
        if ($_POST["site_id"] && $_POST["widget_id"]) {
          ...
          $mc_pop_mm_email = $account_info['email'];
          $mc_pop_mm_password = $account_info['password'];
          $mm = new MailchimppopupMailmunchApi($account_info['email'], $account_info["password"], "http://".MAILCHIMP_POPUP_MAILMUNCH_URL);
        ...
        if (!empty($site->id)) {
          $mc_pop_mm_data = unserialize(get_option("mc_pop_mm_data"));
          $mc_pop_mm_data["site_id"] = $site->id;
          $mc_pop_mm_data["script_src"] = $site->javascript_url;
          update_option("mc_pop_mm_data", serialize($mc_pop_mm_data));
        }
      }
    }
    ...
    if (get_option("mc_pop_mm_user_email") == "") {
      $mc_pop_mm_email = "guest_".uniqid()."@mailmunch.co";
      $mc_pop_mm_password = uniqid();
      $mm = new MailchimppopupMailmunchApi($mc_pop_mm_email, $mc_pop_mm_password, "http://".MAILCHIMP_POPUP_MAILMUNCH_URL);
      $mm->createGuestUser();
      update_option("mc_pop_mm_user_email", $mc_pop_mm_email);
      update_option("mc_pop_mm_user_password", base64_encode($mc_pop_mm_password));
      update_option("mc_pop_mm_guest_user", true);
    }
    ...
    $mc_pop_mm_guest_user = get_option("mc_pop_mm_guest_user");


    if ($mc_pop_mm_guest_user) {
    ...
    if (isset($mc_pop_mm_data["site_id"])) {
      // If there's a site already chosen, we need to get and save it's script_src in WordPress
      $site = $mm_helpers->getSite($sites, $mc_pop_mm_data["site_id"]);
      
      if ($site) {
        $mc_pop_mm_data = array_merge(unserialize(get_option('mc_pop_mm_data')), array("script_src" => $site->javascript_url));
        update_option("mc_pop_mm_data", serialize($mc_pop_mm_data));
      } else {
        // The chosen site does not exist in the mailmunch account any more, remove it locally
        $site_not_found = true;
        $mc_pop_mm_data = unserialize(get_option('mc_pop_mm_data'));
        unset($mc_pop_mm_data["site_id"]);
        unset($mc_pop_mm_data["script_src"]);
        update_option("mc_pop_mm_data", serialize($mc_pop_mm_data));
      }
    }
    ...
    if (!isset($mc_pop_mm_data["site_id"])) {
      ...
      if (sizeof($sites) == 1 && ($sites[0]->name == get_bloginfo() || $sites[0]->name == "WordPress")) {
        ...
        if ($site) {
          $mc_pop_mm_data = array_merge(unserialize(get_option('mc_pop_mm_data')), array("site_id" => $site->id, "script_src" => $site->javascript_url));
          update_option("mc_pop_mm_data", serialize($mc_pop_mm_data));
        }
    ...
    <div id="choose-site-form">
      <form action="" method="POST">
        <div class="form-group">
          ...
          <select name="mc_pop_mm_data[site_id]">
...
<?php
    ...
    $request = $mm->getWidgetsHtml($mc_pop_mm_data["site_id"]);
    $widgets = $request['body'];
    $widgets = str_replace("{{EMAIL}}", $mc_pop_mm_email, $widgets);
    $widgets = str_replace("{{PASSWORD}}", $mc_pop_mm_password, $widgets);
    echo $widgets;
...
<div id="signup-signin-box" style="display:none;">
    ...
    <div class="form-container">
      ...
      <form action="" method="POST">
        ...
        <div class="form-group">
          ...
          <input type="text" placeholder="Site Name" name="site_name" value="<?php echo $mc_pop_mm_data["site_name"] ?>" class="form-control">
        </div>

        <div class="form-group">
          <label>Wordpress URL</label>
          <input type="text" placeholder="Site URL" name="site_url" value="<?php echo $mc_pop_mm_data["site_url"] ?>" class="form-control">
        </div>
  ...
  <div id="sign-in-form" class="<?php if ($_POST && ($_POST["action"] == "sign_in" || $_POST["action"] == "unlink_account")) { ?> active<?php } ?>">
    ...
    <p>Forgot your password? <a href="http://<?php echo MAILCHIMP_POPUP_MAILMUNCH_URL; ?>/users/password/new" target="_blank">Click here</a> to retrieve it.</p>
